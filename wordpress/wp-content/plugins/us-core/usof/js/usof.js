...
! function( $, undefined ) {
	...
	$usof.field[ 'upload' ] = {
		...
		init: function( options ) {
			...
			this.$btnAdd = this.$row.find( '.usof-icon-add' );
		...
		_initDynamicBtns: function() {
			this.$btnsDeleteImage = this.$row.find( '.usof-icon-delete' );
		...
		_dragStart: function( event ) {
			...
			if ( this._hasClass( event.target, 'usof-icon-delete' ) ) {
				return;
			}
			event.stopPropagation();
				...
				$.each( files, function( index, file ) {
					...
					var attachment = wp.media.attachment( parseInt( file ) ),
						renderAttachmentImage = function() {
							...
							html = html
								+ '<div class="usof-icon-delete" title="' + this.i18n.delete + '"></div>'
								+ '</div>';
	...
	$usof.Group.prototype = {
		...
		init: function( elm, options ) {
			...
			this.$field.on( 'click', '.usof-icon-duplicate', this.duplicateGroup.bind( this ) );

			this.$field.on( 'click', '.usof-form-group-item-controls > .usof-icon-delete', function( event ) {
				event.stopPropagation();
			...
			if ( this.isSortable ) {
				...
				this.$field.on( 'mousedown', '.usof-icon-move', this._dragStart.bind( this ) );
	...
	$usof.mixins.Fieldset = {
		...
		initFields: function( $container ) {
			...
			for ( var fieldName in this.showIfDeps ) {
				...
				// Apply ShowIF conditions after field init
