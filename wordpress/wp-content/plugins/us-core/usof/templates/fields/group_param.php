...
if ( ! empty( $field['show_controls'] ) ) {
	...
	if ( ! empty( $field['is_sortable'] ) ) {
		$output .= '<div class="usof-icon-move" title="' . us_translate( 'Move' ) . '"></div>';
	}

	// Show "Duplicate" button, if "is_duplicate" is set
	if ( ! empty( $field['is_duplicate'] ) ) {
		$output .= '<div class="usof-icon-duplicate" title="' . us_translate( 'Duplicate' ) . '"></div>';
	}
	$output .= '<div class="usof-icon-delete" title="' . us_translate( 'Delete' ) . '"></div>';
	$output .= '</div>';
}
