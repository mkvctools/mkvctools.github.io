...
if ( ! us_amp() ) {
	...
	if ( $animation == 'cubeflip' ) {
		...
		$front_bgcolor = ( ! empty( $front_bgcolor ) )
			...
			: 'var(--color-content-bg-alt-grad)';

		// Top & bottom flank with shaded color
		if ( in_array( $direction, array( 'ne', 'e', 'se', 'sw', 'w', 'nw' ) ) ) {
			$shaded_color = us_shade_color( us_get_color( $front_bgcolor, /* Gradient */ TRUE, FALSE ) );
		...
		if ( in_array( $direction, array( 'n', 'ne', 'se', 's', 'sw', 'nw' ) ) ) {
			$shaded_color = us_shade_color( us_get_color( $front_bgcolor, /* Gradient */ TRUE, FALSE ), 0.1 );
