...
// Add color vars from Theme Options
echo ':root {';
...
// Add some predefined vars, needed to simplify CSS values globally
...
if ( ! us_get_option( 'disable_block_editor_assets', FALSE ) ) {
...
for ( $i = 1; $i <= 6; $i ++ ) {
	...
	if ( is_array( us_get_option( 'h' . $i . '_transform' ) ) ) {
		if ( us_in_array( 'italic', us_get_option( 'h' . $i . '_transform' ) ) ) {
			$css .= 'font-style: italic;';
		}
		if ( us_in_array( 'uppercase', us_get_option( 'h' . $i . '_transform' ) ) ) {
...
/* Columns width regarding columns_stacking_width */
<?php
...
if ( us_is_asset_used( 'buttons' ) AND $btn_styles = us_get_option( 'buttons' ) ) {
	...
	echo 'font-style:' . ( us_in_array( 'italic', $btn_styles[0]['text_style'] ) ? 'italic' : 'normal' ) . ';';
	echo 'text-transform:' . ( us_in_array( 'uppercase', $btn_styles[0]['text_style'] ) ? 'uppercase' : 'none' ) . ';';
	...
	echo 'box-shadow: 0 ' . floatval( $btn_styles[0]['shadow'] ) / 2 . 'em ' . $btn_styles[0]['shadow'] . ' ' . us_get_color( $btn_styles[0]['color_shadow'] ) . ';';
	echo 'background:' . (
	...
	echo 'box-shadow: 0 ' . floatval( $btn_styles[0]['shadow_hover'] ) / 2 . 'em ' . $btn_styles[0]['shadow_hover'] . ' ' . us_get_color( $btn_styles[0]['color_shadow_hover'] ) . ';';
	echo 'background:' . (
	...
	foreach ( $btn_styles as $btn_style ) {
		...
		echo 'font-style:' . ( us_in_array( 'italic', $btn_style['text_style'] ) ? 'italic' : 'normal' ) . ';';
		echo 'text-transform:' . ( us_in_array( 'uppercase', $btn_style['text_style'] ) ? 'uppercase' : 'none' ) . ';';
		...
		if ( ! empty( $btn_style['shadow'] ) ) {
			echo 'box-shadow: 0 ' . floatval( $btn_style['shadow'] ) / 2 . 'em ' . $btn_style['shadow'] . ' ' . us_get_color( $btn_style['color_shadow'] ) . ';';
		} else {
		...
		echo 'box-shadow: 0 ' . floatval( $btn_style['shadow_hover'] ) / 2 . 'em ' . $btn_style['shadow_hover'] . ' ' . us_get_color( $btn_style['color_shadow_hover'] ) . ';';
		echo 'background:' . (
